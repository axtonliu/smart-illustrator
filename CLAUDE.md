...
Style-lock 功能使用 `EXTEND.md` 存储用户配置：

| 位置 | 优先级 | 说明 |
|------|--------|------|
| 项目级 `.smart-illustrator/EXTEND.md` | 高 | 项目特定配置 |
| 用户级 `~/.smart-illustrator/EXTEND.md` | 低 | 全局默认配置 |

### 同步规则

- **项目级 EXTEND.md 不同步**：用户配置是项目特定的，不应复制到 skills 目录
- **模板文件需要同步**：`templates/EXTEND.md.template` 需要同步到 skills 目录
- 用户使用 `--save-style` 时，配置保存到项目目录或用户目录，不是 skills 目录

### 检查清单

修改 Style-lock 相关功能时：
- [ ] `templates/EXTEND.md.template` 是否更新？
- [ ] SKILL.md 中的参数说明是否更新？
- [ ] generate-image.ts 中的参数解析是否匹配？
